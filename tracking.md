# Manufacturer Coverage Tracker

| Make | Priority Models / Generations | NA Production Years (Current Gen) | Hybrid Availability | Diesel Availability | `vehicles.json` Coverage |
| --- | --- | --- | --- | --- | --- |
| Chevrolet | Silverado 1500 (T1XX), Equinox (D2XX refresh), Traverse (C1-2), Trailblazer (A2XX) | 2018–present | Silverado EV planned; Equinox EV launching (no conventional hybrid) | 3.0L Duramax LM2 on Silverado | Complete — dataset now includes Suburban gens 1–12, Bel Air gens 1–8, Impala gens 1–10, Chevy II/Nova gens 1–5, El Camino gens 1–5, Monte Carlo through 2007, Malibu 5th–8th, Camaro 1st–6th, late G-van, Tahoe GMT400–T1XX, plus Astro, Kodiak, Lumina APV, Tracker, Express GMT600, Venture, Silverado GMT800/EV/Medium Duty, Avalanche, TrailBlazer, SSR, Colorado GMT355/31XX, Equinox Theta/D2XX, Uplander, HHR, Cobalt, Cruze, Volt, Bolt EV, Traverse Lambda/C1XX, Trax, BrightDrop Zevo; proceed to GMC audit next. |
| GMC | Sierra 1500 (T1XX), Yukon (GMT T1XX), Acadia (C1), Terrain (D2XX) | 2018–present | Mild hybrid discontinued; EV Hummer covers electrified need | 3.0L Duramax LM2 on Sierra | Pending |
| Buick | Encore GX (VSS-F), Envision (E2), Envista (VSS-F), Enclave (Lambda2) | 2020–present | None | None | Pending |
| Cadillac | CT4/CT5 (Alpha2), XT5 (C1), Lyriq (BEV3) | 2019–present | Lyriq BEV, upcoming Optiq | None | Pending |
| Ford | F-150 (P702), Bronco (U725), Explorer (U625), Maverick (C2) | 2021–present | PowerBoost hybrid (F-150, Maverick) | Power Stroke diesel discontinued 2021 | Partial (11th-gen F-150 entry) |
| Lincoln | Navigator (U554), Aviator (CD6), Nautilus (China-built CD6), Corsair (C2) | 2020–present | Aviator/Corsair plug-in hybrids | None | Pending |
| Ram | 1500 (DT), 1500 Classic (DS), Heavy Duty (DJ/D2), ProMaster (X2/X3) | 2019–present | Upcoming Ramcharger PHEV | Cummins 6.7L in HD trucks | Pending |
| Dodge | Charger/Challenger (LD/LA), Durango (WD), Hornet (KL), future Charger Daytona | 2011–2023 (legacy), 2023–present (Hornet) | Hornet R/T PHEV | None | Pending |
| Chrysler | Pacifica (RU), Voyager (RU), 300 (LD) | 2017–present (Pacifica), 2011–2023 (300) | Pacifica Hybrid | None | Pending |
| Jeep | Wrangler (JL), Grand Cherokee (WL), Grand Wagoneer/Wagoneer (WS), Gladiator (JT) | 2018–present | 4xe plug-in hybrids on Wrangler/Grand Cherokee | EcoDiesel discontinued 2023 | Partial (JK Wrangler entry) |
| Tesla | Model S (Plaid), Model 3 (Highland), Model X, Model Y, Cybertruck | 2021–present refreshes | Full BEV lineup | None | Pending |
| Toyota | RAV4 (XA50), Camry (XV80), Tacoma (N400), Tundra (XK70) | 2018–present | Hybrid variants (RAV4, Camry 2025, Tacoma i-FORCE MAX) | Diesel not offered in NA | Pending |
| Lexus | RX (AL20 refresh), GX (J250), TX (KA3), NX (AZ20) | 2022–present | Extensive hybrid/PHEV offerings | None | Pending |
| Honda | CR-V (RS3), Accord (CV3), Civic (FL1/FE), Pilot (YG3) | 2022–present | CR-V/Accord hybrids, Prologue BEV | None | Partial (8th-gen Civic entry) |
| Acura | MDX (YD4), RDX (TC1), Integra (DE4), ZDX (GM BEV3) | 2021–present | ZDX BEV, MDX Type S mild hybrid (planned) | None | Pending |
| Nissan | Rogue (T33), Pathfinder (R53), Frontier (D41), Altima (L34) | 2021–present | e-POWER not sold in NA; Ariya BEV | Titan XD Cummins discontinued 2019 | Partial (L32 Altima entry) |
| Infiniti | QX60 (L51B), QX80 (Z62), QX50 (J55), Q50 (V37) | 2014–present (latest refresh 2022) | Hybrid trims discontinued (Q50/Q70) | None | Pending |
| Mazda | CX-30 (DM), CX-5 (KF), CX-50 (HA), CX-90 (KK), Mazda3 (BP) | 2019–present | PHEV on CX-90, mild hybrid upcoming | Diesel not sold in NA | Pending |
| Subaru | Crosstrek (GU), Outback (BT), Forester (SK), Ascent (WM) | 2018–present | Solterra BEV | Diesel not sold in NA | Pending |
| Mitsubishi | Outlander (GN0W), Eclipse Cross (GK), Mirage (A00/A10) | 2014–present (latest refresh 2022) | Outlander PHEV | None | Pending |
| Hyundai | Tucson (NX4), Santa Fe (MX5), Palisade (LX2), Ioniq 5 (NE) | 2020–present | HEV/PHEV/BEV options across lineup | None | Pending |
| Kia | Sportage (NQ5), Sorento (MQ4), Telluride (ON), EV9 (MV) | 2021–present | HEV/PHEV/BEV options across lineup | None | Pending |
| Genesis | GV70 (JK1), GV80 (JX1), G70 (IK), Electrified G80 (RG3 BEV) | 2018–present | BEV (GV60, Electrified GV70/G80) | None | Pending |
| Volkswagen | Atlas (CA1), Tiguan (BW2), ID.4 (E114), Golf GTI/R (MK8) | 2017–present (facelift 2024) | ID.4 BEV | Diesel discontinued 2015 | Pending |
| Audi | Q5 (FY), Q7 (4M), Q8 e-tron (GE), A4 (B9) | 2017–present (facelifts 2021+) | PHEV/BEV lineup (Q5 TFSI e, Q4 e-tron) | Diesel not sold in NA | Pending |
| BMW | X3 (G01), X5 (G05), 3 Series (G20), i4 (G26 BEV) | 2019–present | PHEV (X5 45e), BEV (i4, iX) | Diesel discontinued 2018 | Pending |
| Mercedes-Benz | GLC (X254), GLE (V167), C-Class (W206), EQE/EQS (V295/X295) | 2020–present | Wide EQ BEV lineup | Diesel not sold in NA passenger line | Pending |
| Mini | Cooper (F56/F66), Countryman (U25), Aceman (U25) | 2024–present | BEV variants (Cooper SE, Countryman Electric) | None | Pending |
| Porsche | Macan (95B/EVP), Cayenne (E3 II), 911 (992), Taycan (J1) | 2019–present | PHEV (Cayenne), BEV (Taycan) | Diesel discontinued 2017 | Pending |
| Volvo | XC60 (SPA), XC90 (SPA2), EX90 (SPA2 BEV), S60 (SPA) | 2018–present | Recharge BEV/PHEV lineup | Diesel not sold in NA | Pending |
| Jaguar | F-Pace (X761), I-Pace (X590), F-Type (X152), future JEA BEV | 2017–present | I-Pace BEV | Diesel not sold in NA | Pending |
| Land Rover | Defender (L663), Range Rover (L460), Range Rover Sport (L461), Discovery (L462) | 2020–present | PHEV (Range Rover/Range Rover Sport), mild hybrid lineup | Diesel limited to fleet import | Pending |
| Alfa Romeo | Giulia (952), Stelvio (949), Tonale (965) | 2017–present | Tonale PHEV | Diesel not sold in NA | Pending |
| Fiat | 500e (332), 500X (334) | 2024–present | BEV (500e) | None | Pending |
| Rivian | R1T, R1S (R1 platform) | 2021–present | Fully BEV | None | Pending |
| Lucid | Air (L21), Gravity (Project Gravity) | 2021–present | Fully BEV | None | Pending |
| Polestar | 2 (PM2), 3 (Polestar 3), 4 (Polestar 4) | 2021–present | Fully BEV | None | Pending |

*TBD manufacturers not yet validated in `NA.txt`: Aston Martin, Bentley, Ferrari, Lamborghini, Maserati, McLaren, Rolls-Royce. Add entries once exotic coverage scope is confirmed.*

## High-Volume Research Priority (Q4 2025)

1. Ford F-Series (P702) — dominant full-size pickup segment; aligns with frequent mobile service requests.
2. Chevrolet Silverado/GMC Sierra (T1XX) — counterpart to F-Series; ensures GM truck coverage.
3. Toyota RAV4 (XA50) — best-selling crossover with hybrid/PHEV complexity.
4. Honda CR-V (RS3) — high-volume compact SUV with hybrid demand.
5. Toyota Camry (XV80 hybrid-only) — staple sedan entering new generation.
6. Chevrolet Equinox & Equinox EV — mainstream compact crossover spanning ICE/EV.
7. Jeep Wrangler (JL) & Gladiator (JT) — off-road platforms with specialized service needs.
8. Tesla Model 3/Model Y — dominant EV pair requiring HV battery service planning.
9. Hyundai/Kia E-GMP trio (Ioniq 5, EV6, EV9) — expanding EV customer base.
10. Toyota Tacoma (N400) — leading midsize pickup with new hybrid i-FORCE MAX.
11. Nissan Rogue (T33) — top-selling Nissan crossover; CVT maintenance focus.
12. Subaru Outback (BT) — prevalent in snowbelt markets; unique body style.
13. Honda Civic (FL1/FE) — compact car mainstay (already partially covered but needs updated gens).
14. Chevrolet Traverse (C1-2) & Buick Enclave — high-demand three-row crossovers.
15. Ram 1500 (DT) & forthcoming Ramcharger — ensure Stellantis truck parity.
